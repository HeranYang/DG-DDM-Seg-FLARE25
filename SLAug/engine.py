...
def prediction_wrapper_pet(model, test_loader, epoch, label_name, mode = 'base', save_prediction = False):
    """
    A wrapper for the ease of evaluation
    Args:
        model:          Module The network to evalute on
    ...
    with torch.no_grad():
        ...
        print("Epoch {} test result on mode {} segmentation are shown as follows:".format(epoch, mode))
        error_dict, dsc_table, domain_names = eval_list_wrapper_pet(out_prediction_list, len(label_name),label_name)
        error_dict["mode"] = mode
        if not save_prediction: # to save memory
            del out_prediction_list
            out_prediction_list = []
...
def eval_list_wrapper(vol_list, nclass, label_name):
    ...
    for organ in range(nclass):
        mean_dc = np.nanmean( dsc_table[:, organ] )
        std_dc  = np.nanstd(  dsc_table[:, organ] )
        print("Organ {} with dice: mean: {:06.5f}, std: {:06.5f}".format(label_name[organ], mean_dc, std_dc))
        error_dict[label_name[organ]] = mean_dc
        overall_vals = dsc_table[:, 1:].reshape(-1)
        overall_mean = np.nanmean(overall_vals)
        overall_std  = np.nanstd( overall_vals)
    print("Overall std dice by sample {:06.5f}".format(overall_std))
    print("Overall mean dice by sample {:06.5f}".format(overall_mean)) # background is noted as class 0 and therefore not counted
    error_dict['overall'] = overall_mean
    ...
    for domain_name, domain_dict in tables_by_domain.items():
        ...
        domain_mean_score = np.nanmean(domain_scores[:, 1:])
