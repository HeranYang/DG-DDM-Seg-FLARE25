...
if __name__ == "__main__":
    ...
    train_loader=DataLoader(data.datasets["train"],
    ...
    val_loader=DataLoader(data.datasets["validation"],
                          batch_size=data.batch_size,
                          num_workers=1)
    ...
    for cur_epoch in range(max_epoch):
        if SBF_config.usage:
            cur_iter = train_one_epoch_SBF(model,
                                           criterion,
                                           train_loader,
                                           opt,
                                           torch.device('cuda'),
                                           cur_epoch,
                                           cur_iter,
                                           optimizer_config.max_iter, SBF_config, visdir)

        else:
            cur_iter = train_one_epoch(model,
                                       criterion,
                                       train_loader,
                                       opt,
                                       torch.device('cuda'),
        ...
        if (cur_epoch+1)%1==0:
            ...
            torch.save({'model': model.state_dict()},os.path.join(ckptdir, f'checkpoint_epoch_{cur_epoch+1}.pth'))
