...
import data.image_transforms as myit
import copy
...
def transform_with_label(aug, add_pseudolabel = False):
    ...
    intensity_tfx1 = get_intensity_transformer(aug)
    intensity_tfx2 = get_intensity_transformer(aug)

    def transform(comp, c_img, c_label, c_plabel, nclass, is_train, use_onehot = False):
        """
        Args
        ...
        if is_train is True:
            ...
            t_img = comp[..., 0 : c_img]
            _label = comp[..., c_img ]
            _pesudolabel = comp[..., c_img+c_label]

            # compact to onehot
            _h_label = np.float32(np.arange( nclass ) == (_label[..., None]) )
            _h_pesudolabel = np.float32(np.arange(nclass) == (_pesudolabel[..., None]))

            comp = np.concatenate( [t_img,
                                    _h_label,
                                    _h_pesudolabel], -1 )
            comp = geometric_tfx(comp)

            # round one_hot labels to 0 or 1
            t_label_h = comp[..., c_img : (c_img+nclass) ]
            t_label_h = np.rint(t_label_h)

            t_pesudolabel_h = comp[..., (c_img + nclass) : (c_img+nclass+nclass) ]
            t_pesudolabel_h = np.rint(t_pesudolabel_h)

            t_img = comp[..., : c_img]

        # intensity transform
        t_img1 = intensity_tfx1(t_img[..., 0: (c_img // 2)])
        t_img2 = intensity_tfx2(t_img[..., (c_img // 2): c_img])

        if use_onehot is True:
            t_label = t_label_h
            t_pesudolabel = t_pesudolabel_h
        else:
            t_label = np.expand_dims(np.argmax(t_label_h, axis = -1), -1)
            t_pesudolabel = np.expand_dims(np.argmax(t_pesudolabel_h, axis=-1), -1)

        return t_img1, t_img2, t_label, t_pesudolabel
