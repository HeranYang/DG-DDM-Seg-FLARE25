...
import segmodel.segloss as segloss
import sys
...
class ExperimentNet(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if opt["continue_train"] or opt["phase"] == 'val':
            try:
        ...
        self.criterionDice = segloss.SoftDiceLoss(self.n_cls).cuda(self.gpu_ids) # soft dice loss
        self.ScoreDice = segloss.SoftDiceScore(self.n_cls, ignore_chan0 = True).cuda(self.gpu_ids) # dice score
        self.ScoreDiceEval = segloss.Efficient_DiceScore(self.n_cls, ignore_chan0 = False).cuda(self.gpu_ids) # for evaluation in 3D

        # using plain CE + Dice loss, not using WCE
        self.criterionWCE = segloss.My_CE(nclass = self.n_cls,\
                batch_size = self.opt["batchSize"], weight = torch.ones(self.n_cls,)).cuda(self.gpu_ids)
    ...
    def set_input(self, input):
        input_img = input['original']
        input_mask = input['target']
        ...
        input_buffer = torch.cat([  input_img for ii in range(3)], dim = 0) # a trick for efficient training from Xu et al.
        
        input_img = input_img.cuda(self.gpu_ids)
        input_buffer = input_buffer.cuda(self.gpu_ids)
        input_mask = input_mask.cuda(self.gpu_ids)
    ...
    def forward_consistency(self, pred_all):
        ...
        if self.opt["consist_type"] == 'kld':
            ...
            #eps = 1e-7
            #pred_all_prob = torch.clamp(F.softmax(pred_all, dim=1), min=eps, max=1.0)
            pred_all_prob = F.softmax(pred_all, dim = 1)
            pred_avg = 1.0 / 3 * ( pred_all_prob[: self._nb_current] + pred_all_prob[self._nb_current : self._nb_current * 2] + pred_all_prob[self._nb_current * 2: ])
            #pred_avg = torch.clamp(pred_avg, min=eps, max=1.0)
            ...
            #pred_all = torch.clamp(F.log_softmax(pred_all, dim=1), min=-100.0, max=0.0)
    ...
    def save(self, rootpath, label):
        self.save_network(self.netSeg, 'Seg', label, self.gpu_ids, rootpath)
